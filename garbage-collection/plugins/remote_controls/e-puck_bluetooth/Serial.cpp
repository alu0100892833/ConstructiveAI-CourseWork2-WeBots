  ...
  mName(name)
{
  ...
  rval = access(name.c_str(), F_OK);
  if (rval != 0) {
    stringstream sstm;
    sstm << "File \"" << name << "\" does not exists. " << strerror(errno);
    throwFatalException(sstm.str());
  }

  // test device file is readable
  rval = access(name.c_str(), R_OK);
  if (rval != 0) {
    stringstream sstm;
    sstm << "File \"" << name << "\" is not readable. " << strerror(errno);
    throwFatalException(sstm.str());
  }

  // test device file is writable
  rval = access(name.c_str(), W_OK);
  if (rval != 0) {
    stringstream sstm;
    sstm << "File \"" << name << "\" is not writable. " << strerror(errno);
    throwFatalException(sstm.str());
  }
  ...
  if (mFd < 0) {
    stringstream sstm;
    sstm << "File \"" << name << "\" cannot be open. " << strerror(errno);
    throwFatalException(sstm.str());
  }
